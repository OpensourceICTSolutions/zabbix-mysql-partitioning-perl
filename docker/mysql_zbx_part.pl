...
unless ( check_have_partition() ) {
	...
	print OUTPUT 'Your installation of MySQL does not support table partitioning.\n';
	exit 1;
}
...
foreach my $key (sort keys %{$tables}) {
	unless (defined($part_tables->{$key})) {
		print OUTPUT 'Partitioning for "'.$key.'" is not found! The table might be not partitioned.\n';
		next;
	}
...
sub create_next_partition {
	...
	for (my $curr_part = 0; $curr_part < $amount_partitions; $curr_part++) {
		...
		if (grep { $_ eq $next_name } keys %{$table_part}) {
			print OUTPUT "Next partition for $table_name table has already been created. It is $next_name\n";
		}
		else {
			print OUTPUT "Creating a partition for $table_name table ($next_name)\n";
...
sub remove_old_partitions {
	...
	foreach my $partition (sort keys %{$table_part}) {
		if ($table_part->{$partition}->{'partition_description'} <= $curr_date->epoch) {
			print OUTPUT "Removing old $partition partition from $table_name table\n";
